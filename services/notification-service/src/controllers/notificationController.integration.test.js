...
describe('notification integration', () => {
    ...
    it('debe obtener notificaciones del usuario con paginacion', async () => {
        const result = await notificationController.getNotificationsByUser(USUARIO_ID, 1, 10);
    ...
    it('debe obtener pagina vacia si no hay mas resultados', async () => {
        const result = await notificationController.getNotificationsByUser(USUARIO_ID, 10, 10);
        assert.equal(result.total, 2);
    ...
    it('debe marcar una notificacion como leida', async () => {
        await notificationController.markAsRead(notifId, USUARIO_ID);
    ...
    it('debe marcar todas las notificaciones como leidas', async () => {
        await notificationController.markAllAsRead(USUARIO_ID);
    ...
    it('debe crear usando plantilla RESERVA_CONFIRMADA', async () => {
        const result = await notificationController.createNotification(
            USUARIO_ID,
            notificationController.TIPOS_NOTIFICACION.RESERVA_CONFIRMADA,
            REF_ID,
            'reserva',
    ...
    it('debe crear usando plantilla NUEVA_CALIFICACION sin data', async () => {
        const result = await notificationController.createNotification(
            USUARIO_ID,
            notificationController.TIPOS_NOTIFICACION.NUEVA_CALIFICACION,
            REF_ID,
            'calificacion'
        );
