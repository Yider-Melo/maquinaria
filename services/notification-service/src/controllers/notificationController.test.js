...
describe('createNotification', () => {
    it('debe crear notificacion usando plantilla', async () => {
        ...
        const result = await notificationController.createNotification(
            'user-id',
            notificationController.TIPOS_NOTIFICACION.NUEVA_RESERVA,
            'ref-1',
            'reserva',
    ...
    it('debe retornar null si el tipo no tiene plantilla', async () => {
        const result = await notificationController.createNotification('user-id', 'tipo_invalido', 'ref-1', 'reserva');
...
describe('createNotificationDirect', () => {
    it('debe lanzar ValidationError si faltan campos', async () => {
        await assert.rejects(
            () => notificationController.createNotificationDirect(null, 'tipo', 'titulo', 'mensaje'),
...
describe('getNotificationsByUser', () => {
    it('debe devolver notificaciones paginadas con conteo de no leidas', async () => {
        ...
        const result = await notificationController.getNotificationsByUser('user-id', 1, 10);
...
describe('markAsRead', () => {
    it('debe marcar notificacion como leida', async () => {
        ...
        const result = await notificationController.markAsRead('n-1', 'user-id');
...
describe('markAllAsRead', () => {
    it('debe marcar todas como leidas', async () => {
        ...
        const result = await notificationController.markAllAsRead('user-id');
...
describe('TIPOS_NOTIFICACION', () => {
    it('debe tener todos los tipos definidos', () => {
        const tipos = notificationController.TIPOS_NOTIFICACION;
