...
describe('create', () => {
    it('debe lanzar ValidationError si faltan campos requeridos', async () => {
        await assert.rejects(
            () => ratingController.create({}, 'user-id'),
...
describe('getByUser', () => {
    it('debe devolver calificaciones de un usuario', async () => {
        ...
        const result = await ratingController.getByUser('user-id');
...
describe('getByMachinery', () => {
    it('debe devolver calificaciones de una maquinaria', async () => {
        ...
        const result = await ratingController.getByMachinery('m-1');
...
describe('getAverage', () => {
    it('debe devolver promedio y total', async () => {
        ...
        const result = await ratingController.getAverage('user-id');
...
describe('update', () => {
    it('debe actualizar puntuacion y comentario', async () => {
        ...
        const result = await ratingController.update('r-1', { puntuacion: 4, comentario: 'Actualizado' }, 'user-id');
    ...
    it('debe lanzar NotFoundError si no existe', async () => {
        ...
        await assert.rejects(
            () => ratingController.update('no-existe', { puntuacion: 3 }, 'user-id'),
    ...
    it('debe lanzar ForbiddenError si no es el autor', async () => {
        ...
        await assert.rejects(
            () => ratingController.update('r-1', { puntuacion: 3 }, 'user-id'),
...
describe('remove', () => {
    it('debe hacer soft-delete', async () => {
        ...
        const result = await ratingController.remove('r-1', 'user-id');
    ...
    it('debe lanzar NotFoundError si no existe', async () => {
        ...
        await assert.rejects(
            () => ratingController.remove('no-existe', 'user-id'),
            (err) => { assert.equal(err.statusCode, 404); return true; }
        );
...
describe('report', () => {
    it('debe marcar como reportado', async () => {
        ...
        const result = await ratingController.report('r-1', 'user-id', 'Spam');
    ...
    it('debe lanzar NotFoundError si no existe', async () => {
        ...
        await assert.rejects(
            () => ratingController.report('no-existe', 'user-id', 'Spam'),
            (err) => { assert.equal(err.statusCode, 404); return true; }
        );
