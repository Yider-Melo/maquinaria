...
describe('payment integration', () => {
    ...
    it('debe crear checkout para reserva confirmada', async () => {
        const result = await paymentController.createCheckout(RESERVA_ID, ARRENDATARIO_ID, 'tarjeta_credito');
    ...
    it('debe rechazar checkout si la reserva no esta confirmada', async () => {
        ...
        await assert.rejects(
            () => paymentController.createCheckout(otraReserva, ARRENDATARIO_ID),
    ...
    it('debe rechazar checkout si no es el arrendatario', async () => {
        await assert.rejects(
            () => paymentController.createCheckout(RESERVA_ID, PROPIETARIO_ID),
            (err) => { assert.equal(err.statusCode, 403); return true; }
        );
    });

    it('debe retornar pago existente si ya hay uno activo', async () => {
        const result = await paymentController.createCheckout(RESERVA_ID, ARRENDATARIO_ID);
    ...
    it('debe obtener pago por ID', async () => {
        const result = await paymentController.getPaymentById(pagoId, ARRENDATARIO_ID);
    ...
    it('debe obtener pagos por reserva', async () => {
        const result = await paymentController.getPaymentsByBooking(RESERVA_ID, ARRENDATARIO_ID);
        assert.equal(result.length, 1);
        assert.equal(result[0].id, pagoId);
    });

    it('debe liberar fondos al propietario', async () => {
        const result = await paymentController.releaseFunds(pagoId);
    ...
    it('debe reembolsar pago', async () => {
        ...
        const result = await paymentController.refund(pagoLiberado);
