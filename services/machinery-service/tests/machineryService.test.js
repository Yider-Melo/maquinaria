...
describe('getById', () => {
    it('debe devolver maquinaria si existe y esta activa', async () => {
        ...
        const result = await machineryService.getById('m-1');
    ...
    it('debe lanzar NotFoundError si no existe', async () => {
        ...
        await assert.rejects(
            () => machineryService.getById('no-existe'),
...
describe('getByOwner', () => {
    it('debe devolver maquinaria del propietario paginada', async () => {
        ...
        const result = await machineryService.getByOwner('owner-id', 1, 10);
...
describe('update', () => {
    it('debe actualizar solo campos permitidos', async () => {
        ...
        await machineryService.update('m-1', { titulo: 'Nuevo titulo', campo_inventado: 'ignorado' }, 'user-id');
    ...
    it('debe lanzar ForbiddenError si no es el propietario', async () => {
        ...
        await assert.rejects(
            () => machineryService.update('m-1', { titulo: 'Nuevo' }, 'other-user'),
...
describe('remove', () => {
    it('debe hacer soft-delete y publicar evento', async () => {
        ...
        const result = await machineryService.remove('m-1', 'user-id');
...
describe('addImage', () => {
    it('debe lanzar ValidationError si no hay url', async () => {
        ...
        await assert.rejects(
            () => machineryService.addImage('m-1', '', 'user-id'),
            (err) => {
    ...
    it('debe marcar primera imagen como portada', async () => {
        ...
        const result = await machineryService.addImage('m-1', 'https://img.com/photo.jpg', 'user-id');
...
describe('deleteImage', () => {
    it('debe lanzar NotFoundError si la imagen no existe', async () => {
        ...
        await assert.rejects(
            () => machineryService.deleteImage('m-1', 'img-no-existe', 'user-id'),
            (err) => { assert.equal(err.statusCode, 404); return true; }
        );
...
describe('getImages', () => {
    it('debe devolver imagenes ordenadas', async () => {
        ...
        const result = await machineryService.getImages('m-1');
...
describe('updateAvailability', () => {
    it('debe hacer upsert de fechas', async () => {
        ...
        const result = await machineryService.updateAvailability('m-1', [
...
describe('getAvailability', () => {
    it('debe lanzar ValidationError sin fechas', async () => {
        await assert.rejects(
            () => machineryService.getAvailability('m-1', null, null),
    ...
    it('debe devolver disponibilidad en rango', async () => {
        ...
        const result = await machineryService.getAvailability('m-1', '2025-01-01', '2025-01-31');
        assert.equal(result.length, 1);
    ...
    it('debe devolver array vacio si no hay registros', async () => {
        ...
        const result = await machineryService.getAvailability('m-1', '2025-06-01', '2025-06-30');
