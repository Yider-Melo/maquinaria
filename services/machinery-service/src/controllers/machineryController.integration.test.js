...
describe('machinery integration', () => {
    ...
    it('debe obtener maquinaria por ID', async () => {
        const result = await machineryController.getById(machineryId);
    ...
    it('debe lanzar 404 si la maquinaria no existe', async () => {
        await assert.rejects(
            () => machineryController.getById('00000000-0000-0000-0000-000000000000'),
            (err) => { assert.equal(err.statusCode, 404); return true; }
        );
    });

    it('debe listar maquinaria del propietario con paginacion', async () => {
        const result = await machineryController.getByOwner(propietarioId, 1, 10);
    ...
    it('debe actualizar maquinaria', async () => {
        const updated = await machineryController.update(machineryId, {
    ...
    it('debe rechazar actualizacion si no es el propietario', async () => {
        await assert.rejects(
            () => machineryController.update(machineryId, { titulo: 'Hackeado' }, 'other-user-id'),
            (err) => { assert.equal(err.statusCode, 403); return true; }
        );
    });

    it('debe agregar y obtener imagenes', async () => {
        const img1 = await machineryController.addImage(machineryId, 'https://img.com/foto1.jpg', propietarioId);
        assert.ok(img1.id);
        assert.equal(img1.es_portada, true);

        const img2 = await machineryController.addImage(machineryId, 'https://img.com/foto2.jpg', propietarioId);
        assert.equal(img2.es_portada, false);

        const images = await machineryController.getImages(machineryId);
    ...
    it('debe eliminar imagen', async () => {
        ...
        await machineryController.deleteImage(machineryId, imageId, propietarioId);

        const remaining = await machineryController.getImages(machineryId);
        assert.equal(remaining.length, 1);
    });

    it('debe lanzar 404 al eliminar imagen inexistente', async () => {
        await assert.rejects(
            () => machineryController.deleteImage(machineryId, '00000000-0000-0000-0000-000000000000', propietarioId),
            (err) => { assert.equal(err.statusCode, 404); return true; }
        );
    });
    ...
    it('debe actualizar y consultar disponibilidad', async () => {
        ...
        const updateResult = await machineryController.updateAvailability(machineryId, fechas, propietarioId);
        assert.equal(updateResult.fechas_actualizadas, 3);

        const availability = await machineryController.getAvailability(machineryId, '2025-07-01', '2025-07-03');
    ...
    it('debe hacer soft-delete de maquinaria', async () => {
        await machineryController.remove(machineryId, propietarioId);
        ...
        await assert.rejects(
            () => machineryController.getById(machineryId),
            (err) => { assert.equal(err.statusCode, 404); return true; }
        );
    });
});
