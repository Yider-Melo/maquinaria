...
describe('login', () => {
    it('debe iniciar sesion con credenciales validas y devolver token JWT', async () => {
        ...
        const result = await authController.login({ email: 'user@test.com', password: 'password123' });
    ...
    it('debe lanzar UnauthorizedError si el email no existe', async () => {
        ...
        await assert.rejects(
            () => authController.login({ email: 'noexiste@test.com', password: 'password123' }),
            (err) => {
    ...
    it('debe lanzar UnauthorizedError si la contrasena es incorrecta', async () => {
        ...
        await assert.rejects(
            () => authController.login({ email: 'user@test.com', password: 'wrongpassword' }),
            (err) => {
                assert.equal(err.statusCode, 401);
                return true;
            }
    ...
    it('debe lanzar UnauthorizedError si el usuario esta inactivo', async () => {
        ...
        await assert.rejects(
            () => authController.login({ email: 'inactivo@test.com', password: 'password123' }),
            (err) => {
                assert.equal(err.statusCode, 401);
                return true;
            }
...
describe('getProfile', () => {
    ...
    it('debe lanzar NotFoundError si el usuario no existe', async () => {
        ...
        await assert.rejects(
            () => authController.getProfile('non-existent-id'),
            (err) => {
...
describe('updateProfile', () => {
    it('debe actualizar solo los campos enviados', async () => {
        ...
        await authController.updateProfile('user-id', { nombre: 'NuevoNombre', telefono: '3009999999' });
    ...
    it('debe retornar el perfil sin cambios si no se envian campos', async () => {
        ...
        const result = await authController.updateProfile('user-id', {});
        assert.equal(result.nombre, 'Juan');
...
describe('2FA', () => {
    it('setup2FA debe generar secreto y QR', async () => {
        ...
        const result = await authController.setup2FA('user-id');
    ...
    it('verify2FA debe marcar como verificado en BD si el codigo es correcto', async () => {
        ...
        await authController.verify2FA('user-id', token);
    ...
    it('verify2FA debe lanzar error con codigo invalido', async () => {
        ...
        await assert.rejects(
            () => authController.verify2FA('user-id', '000000'),
            (err) => {
                assert.equal(err.statusCode, 401);
...
describe('forgotPassword / resetPassword', () => {
    it('forgotPassword debe devolver solo success cuando el email existe', async () => {
        ...
        const result = await authController.forgotPassword('user@test.com');
    ...
    it('forgotPassword debe devolver success aunque el email no exista', async () => {
        ...
        const result = await authController.forgotPassword('noexiste@test.com');
        assert.deepEqual(result, { success: true });
    ...
    it('resetPassword debe actualizar la contrasena con token valido', async () => {
        ...
        const result = await authController.resetPassword('valid-token', 'newpassword123');
        assert.deepEqual(result, { success: true });
    ...
    it('resetPassword debe lanzar error con token invalido', async () => {
        ...
        await assert.rejects(
            () => authController.resetPassword('invalid-token', 'newpassword123'),
            (err) => {
...
describe('validateToken', () => {
    ...
    it('debe devolver null con token invalido', async () => {
        const result = await authController.validateToken('token-invalido');
