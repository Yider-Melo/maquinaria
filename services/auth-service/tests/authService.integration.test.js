...
describe('auth integration', () => {
    ...
    it('debe obtener perfil del usuario', async () => {
        ...
        const profile = await authService.getProfile(userResult.rows[0].id);
    ...
    it('debe generar y usar token de recuperacion de contrasena', async () => {
        const result = await authService.forgotPassword('integracion@test.com');
        ...
        await authService.resetPassword(token, 'nuevapassword456');
    ...
    it('debe validar token JWT', async () => {
        ...
        const valid = await authService.validateToken(loginResult.token);
        assert.equal(valid.email, 'integracion@test.com');

        const invalid = await authService.validateToken('token-falso');
