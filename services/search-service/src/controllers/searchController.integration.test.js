...
describe('search integration', () => {
    ...
    it('debe filtrar por texto en titulo', async () => {
        const result = await searchController.search({ q: 'retroexcavadora' });
        assert.equal(result.pagination.total, 1);
        assert.equal(result.data[0].titulo, 'Retroexcavadora CAT 320');
    });

    it('debe filtrar por texto en descripcion', async () => {
        const result = await searchController.search({ q: 'construccion' });
        assert.equal(result.pagination.total, 1);
    });

    it('debe filtrar por tipo exacto', async () => {
        const result = await searchController.search({ tipo: 'Excavadora' });
        assert.equal(result.pagination.total, 2);
    });

    it('debe filtrar por rango de precio', async () => {
        const result = await searchController.search({ minPrice: 400000, maxPrice: 600000 });
        assert.equal(result.pagination.total, 3);
    });

    it('debe filtrar por ciudad (case insensitive)', async () => {
        const result = await searchController.search({ ciudad: 'villavicencio' });
        assert.equal(result.pagination.total, 1);
    });

    it('debe filtrar por departamento', async () => {
        const result = await searchController.search({ departamento: 'Meta' });
        assert.equal(result.pagination.total, 1);
    });

    it('debe ordenar por precio ascendente (default)', async () => {
        const result = await searchController.search({ sort: 'price_asc' });
    ...
    it('debe ordenar por precio descendente', async () => {
        const result = await searchController.search({ sort: 'price_desc' });
        const prices = result.data.map(m => parseFloat(m.precio_por_dia));
        for (let i = 1; i < prices.length; i++) {
            assert.ok(prices[i] <= prices[i - 1]);
        }
    ...
    it('debe paginar resultados', async () => {
        const result = await searchController.search({ page: 1, size: 2 });
    ...
    it('debe obtener sugerencias de autocompletado', async () => {
        const suggestions = await searchController.getSuggestions('exc');
        assert.ok(suggestions.length >= 1);
        assert.ok(suggestions[0].toLowerCase().includes('exc'));
    });

    it('debe devolver array vacio para sugerencias sin match', async () => {
        const suggestions = await searchController.getSuggestions('zzzzzz');
        assert.deepEqual(suggestions, []);
    });

    it('debe buscar por cercania geografica', async () => {
        const result = await searchController.getNearby(4.14, -73.62, 10);
    ...
    it('debe indexar nueva maquinaria manualmente', async () => {
        ...
        const result = await searchController.search({ q: 'Montacargas' });
        assert.equal(result.pagination.total, 1);
    });

    it('debe eliminar maquinaria del indice', async () => {
        await searchController.removeFromIndex('a1000006-0000-0000-0000-000000000006');
