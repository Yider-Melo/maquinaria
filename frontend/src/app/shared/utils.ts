...
export function estadoPagoLabel(state: string): string {
  const labels: Record<string, string> = {
    pendiente: 'Pendiente', procesando: 'Procesando', retenido: 'En garantía',
    liberado: 'Facturado', reembolsado: 'Reembolsado', fallido: 'Fallido'
  };
  return labels[state] || estadoLabel(state);
}
